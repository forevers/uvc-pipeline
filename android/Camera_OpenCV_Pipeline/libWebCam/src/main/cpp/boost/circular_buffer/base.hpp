...
/*! \endcond */
{
  ...
  // Requirements
    ...
    // Required if the csi_circular_buffer will be compared with anther container.
...
// Helper methods
    ...
    void check_position(size_type index) const {
        if (index >= size())
            throw_exception(std::out_of_range("csi_circular_buffer"));
    }
    ...
    pointer allocate(size_type n) {
        if (n > max_size())
            throw_exception(std::length_error("csi_circular_buffer"));
#if BOOST_CB_ENABLE_DEBUG
